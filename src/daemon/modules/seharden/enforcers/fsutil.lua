...
function M.read_lines(path, context, deps, opts)
    opts = opts or {}

    local attr = deps.lfs_attributes and deps.lfs_attributes(path) or nil
    if attr and attr.mode ~= 'file' then
        return nil, string.format("%s: path '%s' is a %s, not a file", context, path, tostring(attr.mode))
    end

    local file, err = deps.io_open(path, 'r')
    if not file then
        if attr or not opts.missing_as_empty then
            return nil, string.format("%s: could not open '%s': %s", context, path, tostring(err))
        end
        return {}, nil, false
    end
    ...
    if stat then
        if deps.fs_chown then
            ...
            if not owner_ok then
                return nil, string.format("%s: cannot restore owner on '%s': %s", context, path, tostring(owner_err))
            end
        end
        ...
        if deps.fs_chmod then
            ...
            if not mode_ok then
                return nil, string.format("%s: cannot restore mode on '%s': %s", context, path, tostring(mode_err))
            end
        end
    end
