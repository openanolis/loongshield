...
function M.get_probe_tasks(rule, field_name)
    if type(rule) ~= 'table' then
        return {}
    end
    return M.normalize_probe_tasks(rule[field_name or 'probes'])
end
...
local function validate_probe_tasks(rule_probes, field_name)
    ...
    for index, task in ipairs(tasks) do
        local task_field_name = uses_single_task_shorthand and field_name or string.format('%s[%d]', field_name, index)
...
local validate_assertion_node

local function validate_assertion_list(nodes, field_name, opts)
    if not is_list(nodes) then
        return nil, string.format('%s must be a list.', field_name)
    end

    for index, child in ipairs(nodes) do
        local ok, err = validate_assertion_node(child, string.format('%s[%d]', field_name, index), opts)
        if not ok then
            return nil, err
        end
    end
...
validate_assertion_node = function(node, field_name, opts)
    opts = opts or {}
    ...
    if shape_count ~= 1 then
        return nil, string.format("%s must define exactly one of 'all_of', 'any_of', or 'compare'.", field_name)
    end
    ...
    if node.all_of ~= nil then
        return validate_assertion_list(node.all_of, field_name .. '.all_of', opts)
    end

    if node.any_of ~= nil then
        return validate_assertion_list(node.any_of, field_name .. '.any_of', opts)
    end
    ...
    if node.compare == 'for_all' then
        if type(node.expected) ~= 'table' then
            return nil, string.format("%s.expected must be an assertion table when compare is 'for_all'.", field_name)
        end
