...
function test_inspect_kernel_parameter_matches_literal_parameter_names()
    local files = {
        ['/boot/grub2/grub.cfg'] = 'linux /vmlinuz quiet kernel.watchdog=1 kernelXwatchdog=0\n',
        ['/etc/default/grub'] = 'GRUB_CMDLINE_LINUX="quiet kernel.watchdog=1"\n',
    }
    ...
    with_dependencies({
        ...
        assert(result.all_configured == true, 'Expected dotted parameter names to match literally')
        assert(result.details.boot[1].value == 1, 'Expected exact dotted parameter value')
    end)
end
