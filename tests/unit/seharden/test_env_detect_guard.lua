...
function test_env_detect_docker_socket()
    ...
    assert(type(reason) == "string" and reason:find("docker.sock", 1, true),
...
function test_env_detect_containerd_socket()
    ...
    assert(type(reason) == "string" and reason:find("containerd.sock", 1, true),
...
function test_env_detect_kubelet_process()
    ...
    assert(type(reason) == "string" and reason:find("kubelet", 1, true),
...
function test_env_detect_dockerenv_file()
    ...
    assert(type(reason) == "string" and reason:find("dockerenv", 1, true),
...
function test_guard_skip_returns_manual_on_container_host()
    ...
    assert(status == "MANUAL",
        "Expected MANUAL when guard triggers, got: " .. tostring(status))
    assert(type(msg) == "string" and msg:find("Container host detected", 1, true),
...
function test_guard_no_skip_on_bare_metal()
    ...
    assert(msg:find("not found", 1, true),
        "Expected 'not found' error, got: " .. tostring(msg))
end
...
function test_guard_nil_guard_proceeds_normally()
    ...
    local status, msg = rule_executor.enforce(rule, {}, false)
    assert(status == "ERROR",
        "Expected ERROR (enforcer not found) without guard, got: " .. tostring(status))
end
...
function test_guard_unknown_probe_returns_manual_fail_close()
    ...
    assert(status == "MANUAL",
        "Expected MANUAL (fail-close) when guard probe is unknown, got: " .. tostring(status))
    assert(msg:find("not found", 1, true),
        "Expected 'not found' in skip message, got: " .. tostring(msg))
end
...
function test_guard_fallback_to_guard_reason()
    ...
    assert(status == "MANUAL",
        "Expected MANUAL when guard triggers, got: " .. tostring(status))
    assert(type(msg) == "string" and msg:find("container runtime detected", 1, true),
