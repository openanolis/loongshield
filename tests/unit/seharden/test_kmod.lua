...
function test_public_kmod_probes_normalize_hyphenated_module_names()
    kmod_probe._test_set_dependencies({
        ctx_new = function()
            return make_ctx({
                loaded = { 'usb_storage' },
    ...
    local loaded = kmod_probe.is_loaded({ name = 'usb-storage' })
    local blacklisted = kmod_probe.is_blacklisted({ name = 'usb-storage' })
    local install_command = kmod_probe.get_install_command({ name = 'usb-storage' })

    assert(loaded.loaded == true, 'Expected loaded module lookup to normalize hyphens')
    assert(blacklisted.blacklisted == true, 'Expected blacklist lookup to normalize hyphens')
    assert(install_command.command == '/bin/false', 'Expected install command lookup to normalize hyphens')
end
...
function test_get_disable_state_normalizes_hyphenated_module_names()
    ...
    assert(
        result.install_command_disabled == true,
