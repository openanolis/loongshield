...
local function make_client()
    return {
        ...
        wireless_enabled = function()
            return true
        end,
        wwan_enabled = function()
            return false
        end,
        close = function() end,
    }
...
function test_find_listening_ports_reports_requested_ports()
    network_probe._test_set_dependencies({
        ...
        io_popen = function(cmd, mode)
            assert(cmd == 'ss -lntuH 2>/dev/null', 'Expected listening port probe to suppress noisy ss stderr output')
...
function test_find_listening_ports_normalizes_socket_details()
    network_probe._test_set_dependencies({
        client_new = function()
            return nil, 'no nm'
        end,
        io_popen = function(cmd)
            assert(cmd == 'ss -lntuH 2>/dev/null', 'Expected listening port probe to query ss')
            return handle_for({
    ...
    local result = network_probe.find_listening_ports({ ports = { 53, 587 } })

    assert(result.count == 2, 'Expected wildcard and malformed rows to be ignored')
    assert(result.details[1].local_address == '[::1]:587', 'Expected bracketed IPv6 address to be preserved')
    assert(result.details[1].host == '::1', 'Expected bracketed IPv6 host to be normalized')
    assert(result.details[1].loopback == true, 'Expected IPv6 loopback to be classified')
    assert(result.details[2].host == '127.0.0.53%lo', 'Expected scoped resolver host to be preserved')
    assert(result.details[2].loopback == true, 'Expected 127.x listener to be loopback')
end
...
function test_inspect_mta_local_only_flags_non_loopback_smtp_ports()
    ...
    assert(result.non_loopback_details[1].host == '0.0.0.0', 'Expected normalized socket host evidence')
...
function test_inspect_mta_local_only_passes_loopback_listeners_and_absent_mta_config()
    ...
    assert(result.local_only == true, 'Expected loopback-only listeners and no detected MTA config to pass')
...
function test_inspect_mta_local_only_rejects_wildcard_and_mixed_config_values()
    ...
    assert(
        result.config_local_only == false,
...
function test_network_probe_can_load_without_nm_module_for_port_checks()
    ...
    assert(
        type(reloaded.find_listening_ports) == 'function',
