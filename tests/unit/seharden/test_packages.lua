...
function test_get_installed_glob_deduplicates_sorted_names()
    setup('bluez-libs\nbluez\nbluez\n')
    local result = packages_probe.get_installed({ name = 'bluez*' })
    assert(result.count == 2, 'Expected duplicate package names to collapse')
    assert(result.details[1].name == 'bluez', 'Expected glob details to stay sorted')
    assert(result.details[2].name == 'bluez-libs', 'Expected each matching name once')
end
