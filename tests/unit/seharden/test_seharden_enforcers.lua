...
function test_packages_install_calls_dnf()
    ...
    packages_enforcer._test_set_dependencies({
        io_popen = function(cmd)
            cmd_run = cmd
            return {
                read = function() return "Last metadata expiration check: 1:00:00 ago.\nNothing to do.\n" end,
    ...
    assert(cmd_run:find("2>&1", 1, true) ~= nil, "Expected package output to be captured instead of inheriting stdout")
end
...
function test_packages_install_surfaces_captured_dnf_failure_output()
    packages_enforcer._test_set_dependencies({
        io_popen = function(cmd)
            assert(cmd:find("dnf install %-y missing%-pkg", 1) ~= nil, "Expected dnf install command")
            return {
                read = function()
                    return "No match for argument: missing-pkg\nError: Unable to find a match: missing-pkg\n"
                end,
                close = function() return nil, "exit", 1 end,
            }
        end,
    ...
    local ok, err = packages_enforcer.install({ name = "missing-pkg" })
    assert(ok == nil, "Expected install failures to be surfaced")
    assert(err:find("Unable to find a match", 1, true) ~= nil, "Expected captured dnf diagnostics in the error")
end
