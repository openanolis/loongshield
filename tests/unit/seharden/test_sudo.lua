...
function test_sudoers_defaults_parser_handles_flags_and_quoted_values()
    local defaults =
        sudoers.parse_defaults_entry([[Defaults env_reset, logfile = "/var/log/sudo, audit.log", !authenticate]])

    assert(defaults.scope == 'global', 'Expected global Defaults entry to be parsed')
    assert(defaults.options[1].name == 'env_reset', 'Expected flag option to be parsed')
    assert(defaults.options[2].name == 'logfile', 'Expected assignment option to be parsed')
    assert(defaults.options[2].value == '/var/log/sudo, audit.log', 'Expected quoted comma to stay in value')
    assert(defaults.options[3].name == 'authenticate', 'Expected negated flag name to be normalized')
    assert(defaults.options[3].negated == true, 'Expected negated flag to be marked')

    local rewritten, changed =
        sudoers.rewrite_defaults_without_flag(defaults.prefix .. ' !authenticate', 'authenticate', true)
    assert(changed == true, 'Expected requested negated flag to be removed')
    assert(rewritten == nil, 'Expected empty Defaults line to be dropped')
end
...
function test_find_nopasswd_entries_reports_authenticate_disabled_defaults()
    with_dependencies({
        ...
        assert(
            result.count == 1,
            'Expected !authenticate Defaults entries to be treated as no-password sudo violations'
        )
        assert(
            result.details[1].reason == 'authenticate_disabled',
...
function test_find_global_reauth_disabled_ignores_scoped_defaults()
    with_dependencies({
        ...
        assert(
            result.details[1].text == 'Defaults authenticate, !authenticate',
...
function test_collect_audit_paths_tracks_root_file_includedir_and_explicit_includes()
    with_dependencies({
        ...
        lfs_attributes = function(path)
            if path == '/etc/sudoers' or path == '/etc/sudoers.host' or path == '/etc/sudoers.d/10-hardening' then
                return { mode = 'file' }
            end
            if path == '/etc/sudoers.d' then
                return { mode = 'directory' }
        ...
        assert(
            result.count == 3,
            'Expected sudo audit path collection to include active root, include, and includedir paths'
        )
...
function test_collect_permission_paths_tracks_included_files_and_directories()
    with_dependencies({
        ...
        assert(
            result.details[4].path == '/etc/sudoers.d/10-hardening',
            'Expected includedir member file to be included'
        )
    end)
end
...
function test_collect_permission_paths_skips_includedir_members_with_dots()
    with_dependencies({
        lfs_attributes = function(path)
            if
                path == '/etc/sudoers'
                or path == '/etc/sudoers.d/10-hardening'
                or path == '/etc/sudoers.d/README.md'
            then
                return { mode = 'file' }
            end
            if path == '/etc/sudoers.d' then
        ...
        for _, detail in ipairs(result.details) do
            assert(
                detail.path ~= '/etc/sudoers.d/README.md',
