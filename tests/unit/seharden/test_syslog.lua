...
function test_inspect_rsyslog_effective_config_reports_open_failure_with_checked_count()
    with_dependencies({
        lfs_attributes = function(path)
        ...
        io_open = function(path)
            assert(path == '/etc/rsyslog.conf', 'Expected only the discovered rsyslog config')
        ...
        assert(result.available == false, 'Expected unreadable rsyslog config to be unavailable')
        assert(result.checked_count == 1, 'Expected discovered unreadable file to be counted')
        assert(result.remote_input_enabled == false, 'Expected unavailable result shape to stay complete')
        assert(result.error:find('permission denied', 1, true), 'Expected open failure diagnostic')
    end)
end
